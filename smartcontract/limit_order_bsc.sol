...
contract VeroxLimitOrder is Ownable {
    ...
    constructor(IPancakeRouter02 _pancakeRouter02) {
    ...
    function createOrder(OrderType orderType, address assetIn, address assetOut, uint assetInOffered, uint assetOutExpected,address[] calldata path, uint executorFee) external payable {
        ...
        if(orderType == OrderType.EthForTokens) {
            require(assetIn == pancakeRouter02.WETH(), "Use WETH as the assetIn");
            ...
            if (orderType == OrderType.TokensForEth) { require(assetOut == pancakeRouter02.WETH(), "Use WETH as the assetOut"); }
            stakeValue = assetInOffered.mul(STAKE_FEE).div(1000);
    ...
    function executeOrder(uint orderId) external returns (uint[] memory) {
        ...
        if (order.orderType == OrderType.EthForTokens) {
            ...
            swapResult = pancakeRouter02.swapExactETHForTokens{value:order.assetInOffered}(order.assetOutExpected, order.path, order.traderAddress, block.timestamp);
            TransferHelper.safeTransferETH(stakeAddress, order.stake);
        } 
        else if (order.orderType == OrderType.TokensForEth) {
            TransferHelper.safeApprove(order.assetIn, address(pancakeRouter02), order.assetInOffered.sub(order.stake));
            swapResult = pancakeRouter02.swapExactTokensForETH(order.assetInOffered.sub(order.stake), order.assetOutExpected, order.path, order.traderAddress, block.timestamp);
            
            TransferHelper.safeTransfer(order.assetIn, stakeAddress, order.stake);
        }
        else if (order.orderType == OrderType.TokensForTokens) {
            TransferHelper.safeApprove(order.assetIn, address(pancakeRouter02), order.assetInOffered.sub(order.stake));
            swapResult = pancakeRouter02.swapExactTokensForTokens(order.assetInOffered.sub(order.stake), order.assetOutExpected, order.path, order.traderAddress, block.timestamp);
            TransferHelper.safeTransfer(order.assetIn, stakeAddress, order.stake);
        }
