...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function swapExactTokensForTokens(
        ...
        uint stakeValue = amounts[0].mul(STAKE_FEE).div(1000);

        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, address(this), stakeValue
        );
        TransferHelper.safeApprove(path[0], stakeAddress, stakeValue);
        TransferHelper.safeTransfer(path[0], stakeAddress, stakeValue);
        ...
        ordersNum++;
        orderBook[orderId] = Order(msg.sender,path[0],path[path.length-1],amountIn,amountOutMin ,orderId,orders.length);
    ...
    function swapTokensForExactTokens(
        ...
        uint orderId = ordersNum;
        ordersNum++;
        orderBook[orderId] = Order(msg.sender,path[0],path[path.length-1],amountInMax,amountOut,orderId, orders.length);
        ordersForAddress[msg.sender].push(orderId);
        orders.push(orderId);
